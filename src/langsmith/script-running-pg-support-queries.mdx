...
<Note>
The UI and API exports require the `organization:manage` permission.

Additionally, the caller's email must either be listed in `USAGE_EXPORT_ADMIN_EMAILS` or `ORG_ADMINS_INSTALLATION_USAGE_EXPORT_ENABLED` must be set to `true`.
