...
<Note>
The UI and API exports require the `organization:manage` permission. The **Usage export** tab is only visible to users whose email is in the `USAGE_EXPORT_ADMIN_EMAILS` list, or when `ORG_ADMINS_INSTALLATION_USAGE_EXPORT_ENABLED` is set to `true`.
</Note>
