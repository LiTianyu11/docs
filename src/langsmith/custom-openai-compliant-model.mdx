...
The **Base URL** should point to the root of your OpenAI-compatible API server. LangSmith appends the `/chat/completions` path automatically, so do not include it in the Base URL.

For example, if your full chat completions endpoint is:
...
| [LiteLLM Proxy](https://github.com/BerriAI/litellm) (local) | `http://localhost:4000/v1` |
...
You can route to a different path prefix if your server uses one. Set the **Base URL** to include the full prefix up to (but not including) `/chat/completions`.

For example, if your server exposes completions at `/api/v2/chat/completions`, set the Base URL to:

```
https://my-server.example.com/api/v2
```

<Note>
The only requirement is that the server responds to `{base_url}/chat/completions` with an OpenAI-compatible response. Any path prefix is supported as long as this contract is met.
</Note>
