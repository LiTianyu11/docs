...
All export methods produce the same data: LangSmith trace counts, LangGraph Platform node usage, and Agent Builder run counts across all workspaces and organizations.

### Export from the UI (recommended)

<Note>
The UI export requires Helm chart version 0.13.25 or later and the `organization:manage` permission.
</Note>
...
<Note>
The **Usage export** tab is only visible to users with the `organization:manage` permission whose email is in the `USAGE_EXPORT_ADMIN_EMAILS` list, or when `ORG_ADMINS_INSTALLATION_USAGE_EXPORT_ENABLED` is set to `true`.
</Note>
...
<Note>
The API export requires Helm chart version 0.13.25 or later and the `organization:manage` permission.
</Note>
...
You can also run SQL scripts directly against your PostgreSQL database to export usage data.
...
To export Agent Builder run counts:
