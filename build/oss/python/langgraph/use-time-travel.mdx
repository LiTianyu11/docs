...
Use [`get_state`](https://reference.langchain.com/python/langgraph/pregel/main/Pregel/get_state) with `subgraphs=True` to access the subgraph's own checkpoint config, then fork from it:
